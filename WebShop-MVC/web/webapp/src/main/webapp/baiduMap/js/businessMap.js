...
// 复杂的自定义覆盖物
function ComplexCustomOverlay(point, text, mouseoverText,url,house_type,auction_end_time,auction_stage,house_unit_price,house_area,fang_tian_xia_projcodeid,community_name){
    this._point = point;
    this._text=text;//地址
    this._text_2=cut_out_house_name(text);//地址修剪一下
    ...
    this._lianjia_http_url="https://cd.lianjia.com/xiaoqu/rs"+community_name;//链家网址
    ...
    this._anjuke_http_url="https://chengdu.anjuke.com/community/?currentcityid=&rd=1&t=3&type=8&sugg=0&kw="+community_name;
}
...
ComplexCustomOverlay.prototype.initialize = function(map){
    ...
    if(this._house_type=="办公"){
        ...
        if(this._community_name){
            t_text=this._community_name;
        }
        else{
            t_text=this._text_2;
        }
    ...
    i_title.innerHTML=this._house_type+" "+this._house_unit_price+" "+this._house_area+" "+t_text+" ";
...
function selectTdzyfSifaPaimaiByLngLat(lng_bianjie_you,lng_bianjie_zuo,lat_bianjie_shang,lat_bianjie_xia){
    $.ajax({
        url:getContextPath()+"/tdzyfSifaPaimaiController/selectTdzyfSifaPaimaiBusiness",
        async: true,
        type:"post",
        dataType:"json",
        data:{
...
function addPoints(data){
    ...
    $.each(data['tdzyfSifaPaimaiList'],function(i,tdzyfsipai){
        ...
        var myCompOverlay = new ComplexCustomOverlay(pt, tdzyfsipai['house_name'],mouseoverTxt,tdzyfsipai['url'],tdzyfsipai['house_type'],tdzyfsipai['auction_end_time'],tdzyfsipai['auction_stage'],house_unit_price,house_area,tdzyfsipai['fang_tian_xia_projcodeid'],tdzyfsipai['community_name']);
