...
public class WeiXinTdzyfSifaPaimaiController extends BaseController {
     ...
     * @param auction_price_list   数组 起拍价最低和最高两个价格
     * @param area_list   面积
     * @return
     */
...
//    }
    ...
    @RequestMapping(value = "/selectTdzyfSifaPaimaiImagesByUrl",produces = "text/html;charset=UTF-8")
    @ResponseBody
    public String selectTdzyfSifaPaimaiImagesByUrl(String url){
        List<TdzyfSifaPaimaiImg> tdzyfSifaPaimaiImgList=tdzyfSifaPaimaiService.selectTdzyfSifaPaimaiImagesByUrl(url);
        ModelAndView mav=new ModelAndView();
        mav.addObject("tdzyfSifaPaimaiImgList",tdzyfSifaPaimaiImgList);
        mav.addObject("list_size",tdzyfSifaPaimaiImgList.size());
        String res="";
        try {
            res = JSON.toJSONString(mav.getModel());
        }catch (Exception e){
